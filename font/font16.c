...
{
    int i, size=strlen(psz_text);
    int dx = 0;

    for (i=0; i<size; i++)
    {
        font_draw_char(x+dx, y, psz_text[i], color);
        
        /* TODO: refactor */
        dx += widtbl_f16[psz_text[i] - 0x20];
    }
...
int font_draw_text_x2(int x, int y, char *psz_text, uint16_t color)
{
    ...
    for (i=0; i<size; i++)
    {
        font_draw_char_x2(x+dx, y, psz_text[i], color);
        dx += widtbl_f16[psz_text[i] - 0x20] * 2;
    }
...
int font_get_text_width(char *psz_text)
{
  ...
  for (i=0; i<size; i++)
      text_width += widtbl_f16[psz_text[i] - 0x20];
...
int font_get_text_width_x2(char *psz_text)
{
  ...
  for (i=0; i<size; i++)
      text_width += widtbl_f16[psz_text[i] - 0x20] * 2;
