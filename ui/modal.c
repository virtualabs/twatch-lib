...
{
  modal_t *p_modal = p_tile->p_user_data;
...
void modal_init(modal_t *p_modal, int x, int y, int width, int height)
{
  ...
  /* Set default border color (white). */
  p_modal->border_color = RGB(0xf, 0xf, 0xf);
...
/**
 * modal_set_border_color()
 * 
 * @brief: initialize a modal dialog box.
 * @param p_modal: pointer to a `modal_t` structure
 * @param color: border color to use with this modal
 **/

void modal_set_border_color(modal_t *p_modal, uint16_t color)
{
  /* Set border color. */
  p_modal->border_color = color;
}